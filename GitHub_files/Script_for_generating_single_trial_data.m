...
% Resist tags ('LD_endog_extrin'); LD between rare Neutral alleles and the 
% Neutral-choosing Choice allele ('LD_endog_choice'). These statistics are 
% recorded in arrays, where each successive array entry enters the value of
% the statistic for a succcessive generation in the trial run. These 
% matrices, when plotted, form the basis of our "single trial" 
% illustrations in Supplementary Figures 3, 4 & 5.
...
LD_endog_choice = zeros(1,T); % Association (LD) between rare Neutral alleles and Neutral-choosing Choice alleles
...
LD_endog_choice(t)  = sum(sum(sum(sum(pop(:,1,:,:,:,t),1),4),5).*(( sum(sum(pop(:,1,:,:,2,t),1),4)  ./ sum(sum(sum(pop(:,1,:,:,:,t),1),4),5) - choice_extrin_single(t)) .*  (sum(sum(sum(pop(:,1,:,:,:,t),1),4),5) - sum((sum(sum(sum(pop(:,1,:,:,:,t),1),4),5)).^2) ))); % LD between rare Neutral alleles and the Neutral-choosing Choice allele
